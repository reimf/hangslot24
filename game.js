 ...
 */
function makeSvgButton(label, cx, cy, extraClass) {
    const g = svgEl('g');
...
function updateSelectedStates() {
    cards.forEach(card => card.el.classList.toggle('selected', card.id === selectedCardId));
    document.querySelectorAll('.op-btn-svg').forEach(btn => btn.classList.toggle('selected', btn.dataset['op'] === selectedOp));
}
function updateUndoBtn() {
    const btn = getSvgEl('undoBtn');
    btn.classList.toggle('disabled', moveHistory.length === 0);
}
function updateOpButtons() {
    document.querySelectorAll('.op-btn-svg').forEach(btn => btn.classList.toggle('disabled', gameOver));
}
function updateNumButtons() {
    cards.forEach(card => card.el.classList.toggle('disabled', gameOver));
}
// ============================================================
// Event Handlers
// ============================================================
...
function onUndo() {
    ...
    const group = getSvgEl('numberButtonsGroup');
    while (group.firstChild)
        group.removeChild(group.firstChild);
    cards = oldCardData.map((cd) => {
...
// ============================================================
function performMergeAnimation(firstCard, secondCard, result, opElRef) {
    animating = true;
    const svg = getSvgEl('padlockSvg');
    // Positions
    ...
    let opCX = firstCX;
    const opCY = ROW2_Y;
    const opEl = opElRef ?? document.querySelector(`.op-btn-svg[data-op="${selectedOp}"]`);
    if (opEl) {
    ...
    secondAnim.onfinish = () => {
        ...
        if (opClone)
            opClone.animate([{ opacity: '1' }, { opacity: '0' }], { duration: fadeDuration, easing: fadeEasing, fill: 'forwards' });
        const fadeAnim = secondCard.el.animate([{ opacity: '1' }, { opacity: '0' }], { duration: fadeDuration, easing: fadeEasing, fill: 'forwards' });
        fadeAnim.onfinish = () => {
            // ── Clean up ──
...
function init() {
    ...
    document.querySelectorAll('.op-btn-svg').forEach(btn => {
    ...
    // Wire undo button
    const undoBtn = getSvgEl('undoBtn');
